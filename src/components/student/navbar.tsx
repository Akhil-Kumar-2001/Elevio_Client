...
const Navbar = () => {
    ...
    const [student, setStudent] = useState(null); // Removed unnecessary type annotation
    const [image, setImage] = useState(null);
    const [initial, setInitial] = useState(null); // Renamed demo to initial for clarity
    ...
    const fetchStudentDetails = async () => {
        if (!studentId) {
            console.log("Student ID is missing! User may not be authenticated.");
            return;
        }
        try {
            setLoading(true);
    ...
    // Example counts (replace with actual data from state or API)
    ...
    return (
        <nav className="flex items-center justify-between p-4 px-10 bg-white border-b border-gray-200">
            ...
            <div className="flex items-center space-x-8 pr-4">
                ...
                <div className="relative">
                    <Link href={`/cart/${studentId}`}className="text-gray-700 hover:text-blue-500">
                        <svg
                            className="w-6 h-6"
                            fill="none"
                            stroke="currentColor"
                    ...
                    {cartCount > 0 && (
                        <span onClick={handleCart} className="absolute -top-2 -right-2 bg-purple-500 text-white text-xs font-semibold rounded-full w-5 h-5 flex items-center justify-center">
                            {cartCount}
                        </span>
                    )}
                </div>
